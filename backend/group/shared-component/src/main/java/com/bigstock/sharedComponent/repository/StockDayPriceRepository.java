...
public interface StockDayPriceRepository extends JpaRepository<StockDayPrice, StockDayPrice.StockDayPriceId> {
	...
	@Query(value = "select sdp.* from bstock.bstock.stock_day_price sdp where sdp.stock_code = :stockCode and sdp.trading_day <= :endDate AND sdp.change NOT LIKE '%X%' AND sdp.change NOT LIKE '%-%' AND sdp.change NOT LIKE '%除息%' and sdp.change not like '%除權%' and sdp.closing_price not like '%-%'  and sdp.closing_price != '' order by sdp.trading_day desc limit 9" , nativeQuery = true)
	List<StockDayPrice> findByStockCodeAndTradingDayBeforEqualLimitNimeth(@Param("stockCode") String stockCode, @Param("endDate") Date endDate);
