	...
	public List<StockDayPrice> grabFromYahoo(String stockCode) {
			...
			for (int attempt = 1; attempt <= maxRetries; attempt++) {
				try {
					...
					entity.setWeekOfYear(
							endOfWeekLocalDate.getYear() + "W" + today.get(IsoFields.WEEK_OF_WEEK_BASED_YEAR));
					...
					if (attempt < maxRetries) {
						try {
							log.info("url: {} , error: {} , Retrying {} after {} seconds...", url, e.getMessage(),
									symbol, delayMillis / 1000);
							Thread.sleep(delayMillis);
						} catch (InterruptedException ie) {
							Thread.currentThread().interrupt();
							log.warn("grabFromYahoo Retry interrupted for {}", symbol);
							return Collections.emptyList();
						}
					} else {
						Log.warn(stockCode + " - grabFromYahoo  Error , so fast skip:" + e.getMessage());
						return Collections.emptyList();
					}
				}
				...
				try {
					...
					if (attempt < maxRetries) {
						try {
							...
							Thread.currentThread().interrupt();
							log.warn("grabMarginTradingAndShortSellingInfoFromYahoo Retry interrupted for {}", symbol);
							return Collections.emptyList();
						}
					} else {
