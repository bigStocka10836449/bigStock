...
public class StockDayPriceNativeQueryService {
	...
	private static final String FIND_K_VALUE_CONTINUE_UNDER_TEWNTY_BY_RANGE =  "SELECT  "
			+ "    ranked_data.stock_code, "
			+ "    MAX(ranked_data.trading_day) AS start_day, "
			+ "    MIN(ranked_data.trading_day) AS end_day, "
			+ "    COUNT(*) AS total_days, "
			...
			+ ") AS ranked_data "
			+ "WHERE rn <= :limit "
			+ "GROUP BY ranked_data.stock_code "
			+ "HAVING COUNT(*) = :limit "
	...
	private static final String FIND_DATE_RANGE_SUM_CHANGE_RATE_QUERY = " select stock_code, total_change_rate from ( "
			+ "	SELECT  "
			+ "    stock_code, "
			+ "    SUM(CAST(change_rate AS NUMERIC)) AS total_change_rate  "
			+ "FROM ( "
			+ "    SELECT  "
			+ "        stock_code, "
			+ "        trading_day, "
			+ "        ROW_NUMBER() OVER (PARTITION BY stock_code ORDER BY trading_day DESC) AS rn, "
			+ "        CAST(change_rate AS NUMERIC) AS change_rate  "
			+ "    FROM  "
			+ "        bstock.bstock.stock_day_price sdp "
			+ "    WHERE  "
			+ "        sdp.trading_day <= :tradingDay "
	...
	private static final String FIND_DATE_RANGE_MA_TREND_QUERY = "SELECT *  "
			...
			+ "        CASE   "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(five_days_ma AS NUMERIC) END) = 0 OR MIN(CASE WHEN rn = :limit THEN CAST(five_days_ma AS NUMERIC) END) = 0 THEN 'same' "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(five_days_ma AS NUMERIC) END) / MIN(CASE WHEN rn = :limit THEN CAST(five_days_ma AS NUMERIC) END) > 1 THEN 'up' "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(five_days_ma AS NUMERIC) END) / MIN(CASE WHEN rn = :limit THEN CAST(five_days_ma AS NUMERIC) END) < 1 THEN 'down' "
			+ "            ELSE 'same' "
			+ "        END AS five_days_slope,  "
			+ "        CASE   "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(ten_days_ma AS NUMERIC) END) = 0 OR MIN(CASE WHEN rn = :limit THEN CAST(ten_days_ma AS NUMERIC) END) = 0 THEN 'same' "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(ten_days_ma AS NUMERIC) END) / MIN(CASE WHEN rn = :limit THEN CAST(ten_days_ma AS NUMERIC) END) > 1 THEN 'up' "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(ten_days_ma AS NUMERIC) END) / MIN(CASE WHEN rn = :limit THEN CAST(ten_days_ma AS NUMERIC) END) < 1 THEN 'down' "
			+ "            ELSE 'same' "
			+ "        END AS ten_days_slope, "
			+ "        CASE   "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(twenty_days_ma AS NUMERIC) END) = 0 OR MIN(CASE WHEN rn = :limit THEN CAST(twenty_days_ma AS NUMERIC) END) = 0 THEN 'same' "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(twenty_days_ma AS NUMERIC) END) / MIN(CASE WHEN rn = :limit THEN CAST(twenty_days_ma AS NUMERIC) END) > 1 THEN 'up' "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(twenty_days_ma AS NUMERIC) END) / MIN(CASE WHEN rn = :limit THEN CAST(twenty_days_ma AS NUMERIC) END) < 1 THEN 'down' "
			+ "            ELSE 'same' "
			+ "        END AS twenty_days_slope, "
			+ "        CASE   "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(sixty_days_ma AS NUMERIC) END) = 0 OR MIN(CASE WHEN rn = :limit THEN CAST(sixty_days_ma AS NUMERIC) END) = 0 THEN 'same' "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(sixty_days_ma AS NUMERIC) END) / MIN(CASE WHEN rn = :limit THEN CAST(sixty_days_ma AS NUMERIC) END) > 1 THEN 'up' "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(sixty_days_ma AS NUMERIC) END) / MIN(CASE WHEN rn = :limit THEN CAST(sixty_days_ma AS NUMERIC) END) < 1 THEN 'down' "
			+ "            ELSE 'same' "
			+ "        END AS sixty_days_slope, "
			+ "        CASE   "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(one_twenty_days_ma AS NUMERIC) END) = 0 OR MIN(CASE WHEN rn = :limit THEN CAST(one_twenty_days_ma AS NUMERIC) END) = 0 THEN 'same' "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(one_twenty_days_ma AS NUMERIC) END) / MIN(CASE WHEN rn = :limit THEN CAST(one_twenty_days_ma AS NUMERIC) END) > 1 THEN 'up' "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(one_twenty_days_ma AS NUMERIC) END) / MIN(CASE WHEN rn = :limit THEN CAST(one_twenty_days_ma AS NUMERIC) END) < 1 THEN 'down' "
			+ "            ELSE 'down' "
			+ "        END AS one_twenty_days_slope, "
			+ "        CASE   "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(two_fourty_days_ma AS NUMERIC) END) = 0 OR MIN(CASE WHEN rn = :limit THEN CAST(two_fourty_days_ma AS NUMERIC) END) = 0 THEN 'same' "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(two_fourty_days_ma AS NUMERIC) END) / MIN(CASE WHEN rn = :limit THEN CAST(two_fourty_days_ma AS NUMERIC) END) > 1 THEN 'up' "
			+ "            WHEN MIN(CASE WHEN rn = 1 THEN CAST(two_fourty_days_ma AS NUMERIC) END) / MIN(CASE WHEN rn = :limit THEN CAST(two_fourty_days_ma AS NUMERIC) END) < 1 THEN 'down' "
			+ "            ELSE 'same' "
