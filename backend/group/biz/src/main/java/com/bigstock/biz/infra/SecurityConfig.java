...
//                auth.requestMatchers(HttpMethod.PUT, publicPaths).permitAll();
					auth.requestMatchers("/gateway/swagger/**", "/api/biz/swagger/**", "/biz/swagger/**","/ws/**",
							"/v3/api-docs/**", "/swagger-ui/**", "/webjars/**", "/auth/**","/device/**","/biz/stockShareholderStructure/**","/biz/financialCalendarImport/**").permitAll();
					...
					for (RolePath rolePath : rolePaths) {
						...
						pathsMap.forEach((method, paths) -> {
							...
							for (String path : paths) {
								...
								ruleMap.computeIfAbsent(key, k -> new HashSet<>()).add(role);
							}
						});
					}
					...
					ruleMap.forEach((key, roles) -> {
						...
						JwtAuthorizationManager authorizationManager = new JwtAuthorizationManager(
								new ArrayList<>(roles));

						auth.requestMatchers(key.method(), key.path()).access(authorizationManager);
					});
	...
	public JwtAuthenticationConverter jwtAuthenticationConverter() {
		...
		converter.setJwtGrantedAuthoritiesConverter(jwt -> {
			...
			if (roles instanceof Collection<?> collection) {
				return collection.stream().map(Object::toString).map(SimpleGrantedAuthority::new)
